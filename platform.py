...
class Door(Platform):
	...
	def render(self, surface, camPos):
		...
		surface.blit(self.text_Code1,(self.x_Start - camPos[0]-self.TextCode_X, self.y_Start - camPos[1] + self.TextCode_Y))
		color = (200,200,0)
		if self.is_correctCode:
			self.text = self.font.render('[' + (' '.join(str(Ccode) for Ccode in self.correctCode) + ']'), True, color)
			surface.blit(self.text,(self.x_Start - camPos[0]-self.TextCorrect_X, self.y_Start - camPos[1] + self.TextCode_Y))
		else:
			self.text = self.font.render('[' + (' '.join(str(code) for code in self.code) + ']'), True, color)
			surface.blit(self.text, (self.x_Start - camPos[0]-self.TextCorrect_X, self.y_Start - camPos[1] + self.TextCode_Y))
		surface.blit(self.iphone, (self.x_Start - camPos[0] - self.Iphone_X, self.y_Start + self.platform_Height - camPos[1] -self.Iphone_Y))
